...
function get_bam_url()
{
    ...
    local bam_url_line=`grep "BAM_URL=" ${profiling_script}`
    local bam_url_string=`echo ${bam_url_line} | sed 's/CASE_ID=//'` #  | sed 's/"//g'
    bam_url=${bam_url_string}
}
...
function main()
{
    ...
    local s3_url="${}"
    local status="QUEUED"
    local table_name="profiling_caseid_queue"
    local db_cred_s3url="${}"
    ...
    for f in script_dir/profiling_*.sh
    do
        get_case_id "${f}"
        get_bam_url "${f}"
        echo ""
        ...
        queue_status_update "${data_dir}" "${QUEUE_STATUS_TOOL}" "${S3_CFG_PATH}" "${DB_CRED_URL}" "${GIT_CWL_REPO}" "${GIT_CWL_HASH}" "${case_id}" "${bam_url}" "QUEUED" "profiling_caseid_queue" #change this table name
        sbatch ${f}
    done
